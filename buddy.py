...
def summarize_user_advice(user_messages):
    ...
    system_message = '''Given a question summarize answers from users, making your response short, concise and maintaining key ideas and topics. Make sure to keep response relevant to the query. Make sure to quote user. Always quote a user (@Username) ALWAYS OR ELSE

Example input Query: "What is the best programming language to learn for future job opportunities?"

Output: Opinions vary, with some advocating for Python due to its growing use in various fields (@Bob), others suggest JavaScript for its ubiquity in web development (@Steve), while a few recommend focusing on Java or C# for enterprise environments (@George)."'''
    ...
    def replace_with_link(match):
        ...
        for post in range(len(posts)):
            ...
            if found:
                break
        print(username)
        return f'<a href="{comment_url}" class="text-blue-500" target="_blank">@{username if username is not "None" else "*User name unavailable*"}</a>'
